...
export default function Registtration() {
  const { updateResponseData } = useContext(ApiContext);
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        const response = await axios.get(`${main_base_url}/Users/getCountryNames`);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!email.match(emailRegex)) {
      showToast("Email is required and email form is not correct ", "#FF0000");
      return;
    }
    if (!contactNo) {
              ...
              <div className="mx-auto">
                <form onSubmit={handleSubmit} className="grid space-y-6" id="myForm">
                  ...
                  <input type="text" name="firstName"
                    ...
                    className="rounded-md px-3 py-3 border text-sm focus:outline-none text-violet-600" style={{ border: '1px solid #D5D5D5' }} />
                  ...
                  <input type="text" id="lnameInput"
                    ...
                    onChange={handleChange}
                    placeholder="Last Name" className="rounded-md px-3 py-3 border text-sm text-violet-600 focus:outline-none" style={{ border: '1px solid #D5D5D5' }} />
                  <label className="text-slate-500 pb-2" style={{ marginBottom: '-15px', marginTop: 11 }}>Email</label>
                  <input type="email" id="emailInput"
                    name="email"
                    value={formValues.email}
                    onChange={handleChange}
                    placeholder="Email" className="rounded-md px-3 py-3 border text-sm text-violet-600 focus:outline-none" style={{ border: '1px solid #D5D5D5' }} />
                  <label className="text-slate-500 pb-2" style={{ marginBottom: '-15px', marginTop: 11 }}>Password</label>
                  <input type="password" id="passwordInput"
                    name="password"
                    value={formValues.password}
                    onChange={handleChange}
                    placeholder="Password" className="rounded-md px-3 py-3 border text-sm text-violet-600 focus:outline-none" style={{ border: '1px solid #D5D5D5' }} />
...
//     const passwordRegex = /(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/;
//     if (!password.match(passwordRegex)) {
//       showToast("Password is required and password has eight characters including one uppercase letter, one lowercase letter, and one number or special character", "#FF0000");
//       return;
//     }
//     if (!confirmPassword) {
//       showToast("Confirm Password is required.", "#FF0000");
...
//       showToast("Form submitted successfully!", "#00FF00");
//       console.log('Success:', response.data);
//       // Set data in localforage
//       // updateUserData(response.data);
//       await localforage.setItem('myData', response.data.userId);
//       await localforage.setItem('registrationdata', response.data);
